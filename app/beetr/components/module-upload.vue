...
import { type UploadFile, type UploadStatus, type UploadProgressEvent, type UploadRawFile } from 'element-plus'
...
<template>
    <el-upload :auto-upload="autoUpload" ref="uploadRef" class="init_upload" :accept="String(currentAcceptFiles)"
        :show-file-list="false" :disabled="props.disabled" :headers="{ 'X-Access-Token': cookieToken }"
        :action="uploadFileUrl" :before-upload="onBeforeUpload" :on-success="onSuccess" :on-error="onError"
        @change="change" :on-progress="onProgress">
        <slot>子组件插槽</slot>
    </el-upload>
